...
#define COLP_USER_WORM 1    //Colorpair für Wurm

// Symbols to display
#define SYMBOL_WORM_INNER_ELEMENT '0'

// Game state codes
#define  WORM_GAME_ONGOING  0   //Ich befinde mich im Spiel in dieser STATE
...
// ********************************************************************************************
// Global variables   //Diese Variablen sind in allen Funktionen abrufbar, deswegen GLobal
// ********************************************************************************************
...
int theworm_wcolor; //Hier sagen wir welche Farbe der Wurm hat
...
void readUserInput(int* agame_state ); //Wir reagieren auf den Tastendruck
int doLevel();
...
void placeItem(int y, int x, chtype symbol, int color_pair); // E ca si mvaddch in int y si int x
int getLastRow();
int getLastCol();

// Functions concerning the management of the worm data
int initializeWorm(int headpos_y, int headpos_x, int dir, int color); //Konstruktor -> Startwerte des Wurms werden berechnet, dupa vine direction si culoare
void showWorm();
...
void readUserInput(int* agame_state ) { //Normal daca schimbi o variable in o functie nu schimbi variabla din afara, aicea o faci
    int ch; // For storing the key codes

    if ((ch = getch()) > 0) {
        // Ca sa functioneze getch() trebe sa fie in non-blocking mode
        ...
        switch(ch) {
            ...
            case 's' : // User wants single step
                nodelay(stdscr, FALSE);  // We simply make getch blocking
                                         // Nodelay cu flase e o doppelte Verneninung, decy insemana delay si jocul astepata bis zum nächsten Befehl
...
void placeItem(int y, int x, chtype symbol, int color_pair) {
    ...
    attron(COLOR_PAIR(color_pair));     // Start writing in selected color
                                        // Ab sofort all diese Farbe benutzen, mehr geht nicht
